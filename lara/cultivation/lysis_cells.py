...
class InitItem(ltbi.LA_ToolBoxItem):
    def __init__(self,parent=None):
        self.icon_text = "Lysis Cells" 
        self.item_name = "LysisCells"
        self.diagram_class = LysisCellsProcess
        self.icon = QtGui.QIcon(':/linux/tasks/lysis_cells')
        super(InitItem, self).__init__(self.icon_text, self.icon, parent)
        
class LysisCellsProcess(lap.LA_SubProcess):
    def __init__(self, experiment=None, position=None ):
        super(LysisCellsProcess, self).__init__(sp_name="LysisCells", experiment=experiment, position=position)
        ...
        growth_anchit = self.createAnchorItem("LysisCells")
    ...
    def createAnchorItem(self, name=""):
        ...
        if name == "LysisCells" :
            ...
            self.lysis_dur_sb.setValue(120)
    ...
    def updateOverview(self):
        inoc_descr ="""The LysisCells subprocess parameters are: """
        self.exp.exp_ov.newPragraph("description","lysp1",inoc_descr)
                
        par_text = "LysisCells time: \t%d min" % self.centr_dur_sb.value()
    ...
    def updateSubProcessCode(self):
        ...
        self.steps_list += [ lap.BeginSubProcess(step_name="lysis_cells", \
                                            ...
                                            description="lysis cells"),
                            lap.BeginLockedTask(step_name="Lysis") ]
        ...
        if self.dispenser_lysis_cb.currentText() == "Agilent Bravo":
            ...
            self.exp.requestContainerFromStream(container_function="LysisBufferTrough", container_type=self.tr_type, lidding_state="U", min_num = 1)
            self.exp.defDefaultStartLocation(container_function="WashBufferTrough", container_type=self.tr_type, lidding_state="U", dev_pos=("Cytomat_2",2,1,"int"))
            self.exp.defDefaultStartLocation(container_function="LysisBufferTrough", container_type=self.tr_type, lidding_state="U", dev_pos=("Cytomat_2",2,2,"int"))
            
            washtr_name = "WashBufferTrough%s%s%i" %(self.tr_type, "U", 1)
            lysistr_name = "LysisBufferTrough%s%s%i" %(self.tr_type, "U", 1)
            
            self.steps_list += [lap.BeginLockedTask(step_name="Bravo"),lap.MovePlate(target_device="Bravo", position=4, container=[washtr_name])]
            curr_position = 6
        ...
        elif self.dispenser_lysis_cb.currentText() == "Thermo Combi":
            self.steps_list += [ lap.BeginLockedTask(step_name="LysisAddWashBuffer")]
            self.steps_list += [lap.Dispense(device="Combi", mode="Prime", prime_volume=2000, liquids_channel=4 ),  lap.BeginParallel()]
            for i in range(self.num_expression_plates):
                cont_name = curr_expl_func + str(i+1)
                self.steps_list += [ lap.BeginThread(), lap.BeginLockedTask(step_name="Combi"), 
                                     lap.MovePlate(target_device="Combi", container=[cont_name]),
                                     lap.Dispense(device="Combi", volume=200, liquids_channel=4, container=[cont_name]),
                                     ...
                                     lap.EndThread() ]
            self.steps_list += [ lap.EndParallel(), lap.Dispense(device="Combi", mode="Empty", volume=1600, liquids_channel=4 ), lap.EndLockedTask(step_name="LysisAddWashBuffer")]
        ...
        if self.dispenser_lysis_cb.currentText() == "Thermo Combi":
            ...
            self.steps_list += [ lap.BeginLockedTask(step_name="LysisAddLysisBuffer"), lap.Dispense(device="Combi", mode="Prime", prime_volume=2000, liquids_channel=5 ), lap.BeginParallel()]
            for i in range(self.num_expression_plates):
                cont_name = curr_expl_func + str(i+1)
                self.steps_list += [ lap.BeginThread(), lap.BeginLockedTask(step_name="Combi"), 
                                     lap.MovePlate(target_device="Combi", container=[cont_name]),
                                     lap.Dispense(device="Combi", volume=200, liquids_channel=5, container=[cont_name]),
                                     lap.Shake(device="Combi", container=[cont_name]),
                                     lap.AutoLoad( "CombiHotel", orientation="L", container=[cont_name]) ,
                                     lap.EndLockedTask(step_name="Combi"),
                                     lap.EndThread() ]
            self.steps_list += [ lap.EndParallel(), lap.Dispense(device="Combi", mode="Empty", volume=1600, liquids_channel=5 ), lap.EndLockedTask(step_name="LysisAddLysisBuffer")]
            
        # resuspension (with or without adding lysis buffer)
        self.steps_list += [lap.BeginLockedTask(step_name="Bravo")]
        curr_position = 6
        if self.dispenser_lysis_cb.currentText() == "Agilent Bravo":
            self.steps_list += [ lap.MovePlate(target_device="Bravo", position=4, container=[lysistr_name]) ]
        for i in range(self.num_expression_plates):
            cont_name = curr_expl_func + str(i+1)
            self.steps_list += [ lap.MovePlate(target_device="Bravo", position=curr_position, container=[cont_name]) ]
            curr_position += 1 
        ...
        if self.dispenser_lysis_cb.currentText() == "Agilent Bravo":  
            self.steps_list += [ lap.MovePlate(target_device="Cytomat_2", container=[lysistr_name]) ]
        self.steps_list += [ lap.EndLockedTask(step_name="Bravo") ]
    ...
    def mouseDoubleClickEvent(self, event):
        self.incub_diag = LysisCellsParameterDialog(parent_qgw=self)
        
class LysisCellsParameterDialog(QtGui.QDialog):
    def __init__(self, parent_qgw = None, parent=None ):
        super(LysisCellsParameterDialog, self).__init__(parent)
        self.parent_qgw = parent_qgw
        
        tabWidget = QtGui.QTabWidget()
        tabWidget.addTab(self.createGeneralTab(), "General")
        tabWidget.addTab(self.createLysisCellsDetailsTab(), "LysisCells Details")
    ...
    def createLysisCellsDetailsTab(self):
