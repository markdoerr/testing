...
# VERSION: 0.2.3
#
# CREATION_DATE: 2014/10/16
# LASTMODIFICATION_DATE: 2014/10/16
#
...
class InitItem(ltbi.LA_ToolBoxItem):
    def __init__(self,parent=None):
        self.icon_text = "SplitGFPAssay" 
        self.item_name = "SplitGFPAssay"
        self.diagram_class = SplitGFPAssayProcess
        self.icon = QtGui.QIcon(':/linux/tasks/assay')
        super(InitItem, self).__init__(self.icon_text, self.icon, parent)
        
class SplitGFPAssayProcess(lap.LA_SubProcess):
    def __init__(self, experiment=None, position=None ):
        super(SplitGFPAssayProcess, self).__init__(sp_name="SplitGFPAssay", experiment=experiment, position=position)

        logging.debug("I am the SplitGFPAssay - my name is: %s" %self.name() )
        self.num_expr_plates = 4
        
        assay_anchit = self.createAnchorItem("SplitGFPAssay")
    ...
    def createAnchorItem(self, name=""):
        ...
        if name == "SplitGFPAssay" :
            ...
            self.assay_dur_sb.setValue(120)
            info_item_hlout2.addWidget(QtGui.QLabel("SplitGFPAssay. dur.:"))
...
#~ 
    ...
    def setAssayPlatesNum(self, num_plates):
        """ slot for changing number of expression plates (used by details dialog) """
        self.num_assay_plates = num_plates
        self.updateAll()
    ...
    def updateSubProcessCode(self):
        ...
        elif self.assay_temp_dsb.value() > 10.0:
            self.incubation_assay_device = "Cytomat1550_2"
        else :
            self.incubation_assay_device = "Cytomat_2"
        ...
        self.exp.requestContainerFromStream(container_function="LysatePlate", container_type=self.lyspl_type, lidding_state="L", min_num = 1) # max 6 plates ! - to be set by GUI
        self.aspl_type = "Greiner96NwFb"
        self.exp.requestContainerFromStream(container_function="AssayPlate", container_type=self.aspl_type, lidding_state="L", min_num = 1) # max 6 plates ! - to be set by GUI
        
        curr_lyspl_func = "LysatePlate" + self.lyspl_type +"L"
        self.num_lysate_plates =  self.exp.container_stream[curr_lyspl_func]
        curr_aspl_func = "AssayPlate" + self.lyspl_type +"L"
        ...
        self.steps_list += [ lap.BeginSubProcess(step_name="SplitGFPAssay", \
                                            interpreter=lap.LA_ProcessElement.momentum32, \
                                            description="splitGFP Assay"),
                                            lap.BeginLockedTask(step_name="SplitGFPAssay") ]
        ...
        for i in range(self.num_lysate_plates):
            ...
            self.steps_list += [lap.BeginThread(), lap.BeginLockedTask(step_name="Bravo"),
            ...
            self.steps_list += [ lap.RunExternalProtocol(target_device="Bravo", protocol="MDO_EnzymeFastTransfer1plate20ulSubstrTrough.pro"), 
                                 lap.Delay(),
                                 lap.MovePlate(target_device="Varioskan", container=[as_cont_name])]
            if self.assay_cb.currentText() == "Kinetic":
                self.steps_list += [ lap.BeginParallel(),  
                                     lap.BeginThread(), 
                                     lap.RunExternalProtocol(target_device="Varioskan", protocol="MDO_splitGFP_fl_kin", container=[as_cont_name]), lap.EndThread(),
                                     lap.MovePlate(target_device="Carousel", container=[as_cont_name]),
                ...
                self.steps_list += [ lap.EndThread()]
            elif self.assay_cb.currentText() == "Single Point":
                self.steps_list += [ lap.BeginParallel(),  
                                     lap.BeginThread(), lap.RunExternalProtocol(target_device="Varioskan", protocol="MDO_splitGFP_fl_sp", container=[as_cont_name]), 
                ...
                self.steps_list += [ lap.MovePlate(target_device="Cytomat_2", container=[as_cont_name]),
                                     lap.EndThread() ]
        self.steps_list += [ lap.EndParallel(), lap.EndLockedTask(step_name="SplitGFPAssay")]
        
        if self.assay_cb.currentText() == "Single Point":
            self.steps_list += [ lap.BeginLoop(max_iterations=self.num_assay_cyc_sb.value(), description="splitGFP assay loop"),
                                 lap.BeginParallel()]
            for i in range(self.num_assay_plates):
                ...
                self.steps_list += [ lap.BeginThread(),
                                      lap.Incubate(device=self.incubation_assay_device, incubation_duration=self.assay_dur_sb.value(), container=[as_cont_name] ), 
                                         ...
                                         lap.RunExternalProtocol(target_device="Varioskan", protocol="MDO_splitGFP_fl_sp", container=[as_cont_name]),
    ...
    def mouseDoubleClickEvent(self, event):
        self.incub_diag = SplitGFPAssayParameterDialog(parent_qgw=self)
        
class SplitGFPAssayParameterDialog(QtGui.QDialog):
    def __init__(self, parent_qgw = None, parent=None ):
        super(SplitGFPAssayParameterDialog, self).__init__(parent)
        self.parent_qgw = parent_qgw
        
        tabWidget = QtGui.QTabWidget()
        tabWidget.addTab(self.createGeneralTab(), "General")
        tabWidget.addTab(self.createSplitGFPAssayDetailsTab(), "SplitGFPAssay Details")
        ...
        self.setWindowTitle("SplitGFPAssay Dialog")
        self.show()
    ...
    def createGeneralTab(self):
        ...
        self.num_assay_plates_sb = QtGui.QSpinBox(container_gb)
        self.num_assay_plates_sb.setRange(2, 32)
        self.num_assay_plates_sb.setValue(4)
        info_item_hlout1.addWidget(QtGui.QLabel("Num. of plates:"))
        info_item_hlout1.addWidget(self.num_assay_plates_sb)
        info_item_hlout1.addWidget(QtGui.QLabel("96 well MTPs"))
        
        self.num_assay_plates_sb.valueChanged.connect(self.parent_qgw.setAssayPlatesNum)
    ...
    def createSplitGFPAssayDetailsTab(self):
        details_tabw = QtGui.QWidget(self)
        
        growth_gb = QtGui.QGroupBox("SplitGFPAssay Details")
